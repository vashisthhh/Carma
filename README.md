...
> **The vehicle itself is the interface to its documented history.**
...
| :--- | :--- | :--- |
| **Document Extraction** | Google Gemini `gemini-2.5-flash` | Extracts structured data from invoices, repair job cards, Insurance policies, and PUC certificates. |
| **Component Classification** | Semantic matching & Gemini | Maps unstructured line items (e.g., *"tyre puncture repair & wheel balancing"*) to physical 3D components (`tyre-front-right`, `tyre-front-left`). |
| **Conversational Intelligence** | Gemini + Grounded RAG Prompt | Answers vehicle questions strictly using verified records; explicitly states when an item is unrecorded without assuming it never occurred. |
| **Deterministic Fallback** | Custom Regex & Pattern Engine | Provides instant extraction of dates, amounts, and component keywords if offline or without an API key. |
...
- **Live Application**: [https://carma-six.vercel.app](https://carma-six.vercel.app/)
