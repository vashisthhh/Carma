 ...
 */
export function decodeAscii85(str) {
 ...
 */
export function understandVehicleDocumentContent(docText, fileName = '', documentCategory = 'insurance') {
  const combined = `${fileName}\n${docText}`.trim();
  const normalized = combined.replace(/[_.\-\/]/g, ' ');
...
/**
 * Handle document extraction request using Gemini 1.5 Flash if API key is present,
 * with graceful fallback to server-side buffer parsing and document understanding.
 */
    ...
    "warning": string | null
      ...
      if (geminiRes.ok) {
        ...
        if (rawText) {
          ...
          return { ...parsed, engine: 'gemini-1.5-flash', rawGeminiResponse: rawText };
        }
      } else {
